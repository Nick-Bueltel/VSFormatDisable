...
function activate(context) {
    ...
    vscode.commands.registerCommand('config.commands.configureViewOnWindowOpen', () => __awaiter(this, void 0, void 0, function* () {
        // 1) Getting the value
        const value = yield vscode.window.showQuickPick(['explorer', 'search', 'scm', 'debug', 'extensions'], { placeHolder: 'Select the view to show when opening a window.' });
        if (vscode.workspace.workspaceFolders) {
            // 2) Getting the Configuration target
            const target = yield vscode.window.showQuickPick([
            ...
            if (value && target) {
                ...
                yield vscode.workspace.getConfiguration().update('conf.view.showOnWindowOpen', value, target.target);
                /*
            ...
            yield vscode.workspace.getConfiguration().update('conf.view.showOnWindowOpen', value, vscode.ConfigurationTarget.Global);
        }
    }));
    ...
    vscode.commands.registerCommand('config.commands.configureEmptyLastLineCurrentFile', () => __awaiter(this, void 0, void 0, function* () {
        if (vscode.window.activeTextEditor) {
            ...
            const value = Object.assign(Object.assign({}, currentValue), { [currentDocument.fileName]: true });
            // 4) Update the configuration
            yield configuration.update('conf.resource.insertEmptyLastLine', value, target);
        }
    }));
    // Example: Updating Resource scoped Configuration
    vscode.commands.registerCommand('config.commands.configureEmptyLastLineFiles', () => __awaiter(this, void 0, void 0, function* () {
        // 1) Getting the value
        const value = yield vscode.window.showInputBox({ prompt: 'Provide glob pattern of files to have empty last line.' });
        if (vscode.workspace.workspaceFolders) {
            ...
            if (value && target) {
                if (target.target === vscode.ConfigurationTarget.WorkspaceFolder) {
                    ...
                    let workspaceFolder = yield vscode.window.showWorkspaceFolderPick({ placeHolder: 'Pick Workspace Folder to which this setting should be applied' });
                    if (workspaceFolder) {
                        ...
                        const currentValue = configuration.get('conf.resource.insertEmptyLastLine');
                        const newValue = Object.assign(Object.assign({}, currentValue), { [value]: true });
                        // 6) Update the configuration value
                        yield configuration.update('conf.resource.insertEmptyLastLine', newValue, target.target);
                    }
                }
                else {
                    ...
                    const currentValue = configuration.get('conf.resource.insertEmptyLastLine');
                    const newValue = Object.assign(Object.assign({}, currentValue), (value ? { [value]: true } : {}));
                    // 3) Update the value in the target
                    yield vscode.workspace.getConfiguration().update('conf.resource.insertEmptyLastLine', newValue, target.target);
                }
            }
        }
        else {
            ...
            yield vscode.workspace.getConfiguration().update('conf.resource.insertEmptyLastLine', newValue, vscode.ConfigurationTarget.Global);
        }
    }));
    let statusSizeDisposable;
    ...
    context.subscriptions.push(vscode.workspace.onDidOpenTextDocument(e => {
        ...
        const showSize = vscode.workspace.getConfiguration('', e).get('conf.language.showSize');
        // 3) If matches, insert empty last line
        if (showSize) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('config.commands.overrideLanguageValue', () => __awaiter(this, void 0, void 0, function* () {
        // 1) Getting the languge id
        const languageId = yield vscode.window.showInputBox({ placeHolder: 'Enter the language id' });
        // 2) Update
